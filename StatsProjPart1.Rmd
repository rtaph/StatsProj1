...
As expected, these two values are close. As was the case with the culumulative means of multiple experiments, we would expect the cumulative variance of multiple experiments to converge to the theoretical variance.
...
The green line represents the identity line for normal distribution. Although the distribution is not perfectly normal, visual inspection reveals that the fit is pretty good. The deviations from normality fall within the expected range of noise for a monte carlo simulation with 40 draws. 
